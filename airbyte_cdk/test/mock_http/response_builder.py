...
def _extract(path: list[str], response_template: dict[str, Any]) -> Any:  # noqa: ANN401
    return functools.reduce(lambda a, b: a[b], path, response_template)


def _replace_value(dictionary: dict[str, Any], path: list[str], value: Any) -> None:  # noqa: ANN401
    current = dictionary
    for key in path[:-1]:
        current = current[key]
...
def _write(dictionary: dict[str, Any], path: list[str], value: Any) -> None:  # noqa: ANN401
    current = dictionary
    for key in path[:-1]:
...
class Path(ABC):
    @abstractmethod
    def write(self, template: dict[str, Any], value: Any) -> None:  # noqa: ANN401
        pass

    @abstractmethod
    def update(self, template: dict[str, Any], value: Any) -> None:  # noqa: ANN401
        pass

    def extract(self, template: dict[str, Any]) -> Any:  # noqa: ANN401, B027
        pass


class FieldPath(Path):
    def __init__(self, field: str):  # noqa: ANN204
        self._path = [field]
    ...
    def extract(self, template: dict[str, Any]) -> Any:  # noqa: ANN401
...
class NestedPath(Path):
    def __init__(self, path: list[str]):  # noqa: ANN204
        self._path = path
...
class FieldUpdatePaginationStrategy(PaginationStrategy):
    def __init__(self, path: Path, value: Any):  # noqa: ANN204, ANN401
        self._path = path
        self._value = value
...
class RecordBuilder:
    ...
    def with_id(self, identifier: Any) -> "RecordBuilder":  # noqa: ANN401
        self._set_field("id", self._id_path, identifier)
        return self

    def with_cursor(self, cursor_value: Any) -> "RecordBuilder":  # noqa: ANN401
        self._set_field("cursor", self._cursor_path, cursor_value)
        return self

    def with_field(self, path: Path, value: Any) -> "RecordBuilder":  # noqa: ANN401
        path.write(self._record, value)
        return self

    def _set_field(self, field_name: str, path: Path | None, value: Any) -> None:  # noqa: ANN401
        if not path:
            raise ValueError(
...
def find_template(resource: str, execution_folder: str) -> dict[str, Any]:
    ...
    with open(response_template_filepath) as template_file:  # noqa: PLW1514, PTH123
