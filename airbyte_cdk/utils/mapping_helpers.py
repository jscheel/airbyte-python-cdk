...
def flatten_mapping(mapping: Mapping[str, Any], prefix: List[str] = None) -> List[tuple[List[str], Any]]:
    """
...
def combine_mappings(
    ...
    * If there are multiple mappings containing keys and one of them is a string
    ...
    non_empty_mappings = [m for m in mappings if m is not None and not (isinstance(m, Mapping) and not m)]
            ...
            for paths2 in all_paths[i+1:]:
                for path2, value2 in paths2:
                    if has_nested_conflict(path1, value1, path2, value2):
                        raise ValueError(f"Duplicate keys or nested path conflict found: {'.'.join(path1)} conflicts with {'.'.join(path2)}")
