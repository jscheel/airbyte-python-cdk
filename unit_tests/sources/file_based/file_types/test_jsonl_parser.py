...
def test_given_limit_hit_when_infer_then_stop_considering_records(stream_reader: MagicMock) -> None:
    jsonl_file_content = (
        '{"key": 2.'
        + "2" * JsonlParser.MAX_BYTES_PER_FILE_FOR_SCHEMA_INFERENCE
        + '}\n{"key": "a string"}'
    )
    stream_reader.open_file.return_value.__enter__.return_value = io.BytesIO(
        jsonl_file_content.encode("utf-8")
    )
...
def test_given_multiline_json_objects_and_hits_read_limit_when_infer_then_return_proper_types(
    ...
    stream_reader.open_file.return_value.__enter__.return_value = (
        JSONL_CONTENT_WITH_MULTILINE_JSON_OBJECTS
    )
    schema = _infer_schema(stream_reader)
...
def test_given_one_json_per_line_when_parse_records_then_return_records(
    stream_reader: MagicMock,
) -> None:
    stream_reader.open_file.return_value.__enter__.return_value = (
        JSONL_CONTENT_WITHOUT_MULTILINE_JSON_OBJECTS
...
def test_given_one_json_per_line_when_parse_records_then_do_not_send_warning(
    stream_reader: MagicMock,
) -> None:
    stream_reader.open_file.return_value.__enter__.return_value = (
        JSONL_CONTENT_WITHOUT_MULTILINE_JSON_OBJECTS
...
def test_given_multiline_json_object_when_parse_records_then_return_records(
    stream_reader: MagicMock,
) -> None:
    stream_reader.open_file.return_value.__enter__.return_value = (
        JSONL_CONTENT_WITH_MULTILINE_JSON_OBJECTS
...
def test_given_multiline_json_object_when_parse_records_then_log_once_one_record_yielded(
    stream_reader: MagicMock,
) -> None:
    stream_reader.open_file.return_value.__enter__.return_value = (
        JSONL_CONTENT_WITH_MULTILINE_JSON_OBJECTS
...
def test_given_unparsable_json_when_parse_records_then_raise_error(
    stream_reader: MagicMock,
) -> None:
