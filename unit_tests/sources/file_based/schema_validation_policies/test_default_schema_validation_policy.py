...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            CONFORMING_RECORD,
            SCHEMA,
            ValidationPolicy.emit_record,
            True,
            id="record-conforms_emit_record",
        ),
        pytest.param(
            NONCONFORMING_RECORD,
            SCHEMA,
            ValidationPolicy.emit_record,
            True,
            id="nonconforming_emit_record",
        ),
        pytest.param(
            CONFORMING_RECORD,
            SCHEMA,
            ValidationPolicy.skip_record,
            True,
            id="record-conforms_skip_record",
        ),
        pytest.param(
            NONCONFORMING_RECORD,
            SCHEMA,
            ValidationPolicy.skip_record,
            False,
            id="nonconforming_skip_record",
        ),
        pytest.param(
            CONFORMING_RECORD,
            SCHEMA,
            ValidationPolicy.wait_for_discover,
            True,
            id="record-conforms_wait_for_discover",
        ),
        pytest.param(
            NONCONFORMING_RECORD,
            SCHEMA,
            ValidationPolicy.wait_for_discover,
            False,
            id="nonconforming_wait_for_discover",
        ),
    ],
)
def test_record_passes_validation_policy(
    ...
    if validation_policy == ValidationPolicy.wait_for_discover and expected_result is False:
        with pytest.raises(StopSyncPerValidationPolicy):
            DEFAULT_SCHEMA_VALIDATION_POLICIES[validation_policy].record_passes_validation_policy(
                record, schema
            )
    else:
        assert (
