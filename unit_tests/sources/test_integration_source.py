...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            "CLOUD",
            "http://unsecured.com/api/v1/",
            [],
            "system_error",
            id="test_cloud_read_with_unsecured_url",
        ),
        pytest.param(
            "CLOUD",
            "https://172.20.105.99/api/v1/",
            [],
            "config_error",
...
def test_external_request_source(
    capsys, deployment_mode, url_base, expected_records, expected_error
):
    source = SourceTestFixture()

    with mock.patch.dict(
        os.environ, {"DEPLOYMENT_MODE": deployment_mode}, clear=False
    ):  # clear=True clears the existing os.environ dict
...
def test_external_oauth_request_source(
    capsys, deployment_mode, token_refresh_url, expected_records, expected_error
):
