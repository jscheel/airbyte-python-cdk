...
def test_request_option_validation():
    ...
    with pytest.raises(
        ValueError, match="RequestOption requires either a field_name or field_path"
    ):
    ...
    with pytest.raises(
        ValueError, match="Nested field injection is only supported for body JSON injection."
    ):
        RequestOption(
            field_path=["data", "field"], inject_into=RequestOptionType.header, parameters={}
        )
...
def test_inject_into_dict_deep_nesting():
    ...
    assert target == {"level1": {"level2": {"level3": {"level4": {"field": "deep_value"}}}}}
...
def test_inject_into_dict_various_value_types():
    ...
    for value, name in test_cases:
        target = {}
        request_option = RequestOption(
            field_path=["data", name], inject_into=RequestOptionType.body_json, parameters={}
        )
...
def test_inject_into_dict_with_interpolation_combinations():
    ...
    assert target == {"data": {"admin": {"12345": {"profile": {"details": "test_value"}}}}}
...
def test_inject_into_dict_error_handling():
    ...
    with pytest.raises(TypeError):
        request_option = RequestOption(
            field_name="test", inject_into=RequestOptionType.body_json, parameters={}
        )
