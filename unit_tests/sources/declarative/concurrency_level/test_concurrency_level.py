...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            "{{ config['num_workers'] or 40 }}",
            75,
            50,
            id="test_default_concurrency_using_interpolation",
        ),
        pytest.param(
            "{{ config['missing'] or 40 }}",
            75,
            40,
            id="test_default_concurrency_using_interpolation_no_value",
        ),
        pytest.param(
            "{{ config['num_workers'] or 40 }}",
            10,
            10,
            id="test_use_max_concurrency_if_default_is_too_high",
        ),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            {"num_workers": "fifty five"},
            None,
            ValueError,
            id="test_invalid_default_concurrency_as_string",
        ),
