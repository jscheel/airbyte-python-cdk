...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            DebugSliceLogger(), logging.DEBUG, True, id="debug_logger_should_log_if_level_is_debug"
        ),
        pytest.param(
            DebugSliceLogger(),
            logging.INFO,
            False,
            id="debug_logger_should_not_log_if_level_is_info",
        ),
        pytest.param(
            DebugSliceLogger(),
            logging.WARNING,
        ...
        pytest.param(
            ...
            logging.ERROR,
            False,
            id="debug_logger_should_not_log_if_level_is_error",
        ),
        pytest.param(
            DebugSliceLogger(),
            logging.CRITICAL,
            False,
            id="always_log_logger_should_not_log_if_level_is_critical",
        ),
        pytest.param(
            AlwaysLogSliceLogger(),
            logging.DEBUG,
            True,
            id="always_log_logger_should_log_if_level_is_debug",
        ),
        pytest.param(
            AlwaysLogSliceLogger(),
            logging.INFO,
            True,
            id="always_log_logger_should_log_if_level_is_info",
        ),
        pytest.param(
            AlwaysLogSliceLogger(),
            logging.WARNING,
        ...
        pytest.param(
            ...
            logging.WARNING,
            True,
            id="always_log_logger_should_log_if_level_is_warning",
        ),
        pytest.param(
            AlwaysLogSliceLogger(),
            logging.ERROR,
            True,
            id="always_log_logger_should_log_if_level_is_error",
        ),
        pytest.param(
            AlwaysLogSliceLogger(),
            logging.CRITICAL,
            True,
            id="always_log_logger_should_log_if_level_is_critical",
        ),
    ],
)
...
def test_create_slice_log_message(_slice, expected_message):
    expected_log_message = AirbyteMessage(
        type=MessageType.LOG, log=AirbyteLogMessage(level=Level.INFO, message=expected_message)
    )
