...
class MockSource(Source):
    def read(
        ...
        logger.info(
            I_AM_A_SECRET_VALUE
            + " plus Some non secret Value in the same log record"
            + NOT_A_SECRET_VALUE
        )
...
def test_airbyte_secret_is_masked_on_logger_output(source_spec, mocker, config, caplog):
    ...
    expected_secret_values = [
        config[k] for k, v in source_spec["properties"].items() if v.get("airbyte_secret")
    ]
    expected_plain_text_values = [
        config[k] for k, v in source_spec["properties"].items() if not v.get("airbyte_secret")
    ]
...
def test_non_airbyte_secrets_are_not_masked_on_uncaught_exceptions(mocker, caplog, capsys):
    ...
    mocker.patch.object(MockSource, "read_catalog", return_value={})
    mocker.patch.object(
        MockSource, "read", side_effect=Exception("Exception:" + NOT_A_SECRET_VALUE)
    )
